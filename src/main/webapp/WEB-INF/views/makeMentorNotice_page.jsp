...
function insert(){
	...
	var formData= new FormData($("#make")[0]);
	formData.append('memberNumber',<%=session.getAttribute("sessionNUMBER")%>);
	formData.append('memberName','<%=session.getAttribute("sessionNAME")%>');
	formData.append('memberBelong','<%=session.getAttribute("sessionTEMPER")%>');
	$.ajax({
		 url :"/toring/addMentorNotice"
		 ,type:"post"
		 ,data:formData
         ,dataType:"json"
         ,processData : false
...
function jusoCallBack(roadFullAddr,roadAddrPart1,addrDetail,roadAddrPart2,engAddr, jibunAddr, zipNo, admCd, rnMgtSn, bdMgtSn,detBdNmList,bdNm,bdKdcd,siNm,sggNm,emdNm,liNm,rn,udrtYn,buldMnnm,buldSlno,mtYn,lnbrMnnm,lnbrSlno,emdNo,entX,entY){
	...
	document.make.roadAddrPart1.value = roadAddrPart1+roadAddrPart2;
	//document.make.roadAddrPart2.value = ;  
	document.make.addrDetail.value = addrDetail;
...
<%
          ...
          <div class="span8">
            ...
            <article>
              <div class="row">
				...
				<table class="bbsWriteA" summary="게시물 쓰기">
					...
					<tbody>
						<tr>
							...
							<td><input id="title" name="title" class="inputTxt01" style="width: 95%;" type="text" maxlength="100"></td>
						</tr>
						<tr>
							...
							<td>
								...
								<input id="recruitSdate" name="recruitSdate" type="text" style="border: 1px solid #e0e0e0; background-color: #fbfbfb;">	
								</label>
								<label style="margin-right: 10px;">~</label>
								<label for="recruitFdate">
								<input id="recruitFdate" name="recruitFdate" type="text" style="border: 1px solid #e0e0e0; background-color: #fbfbfb;">
								</label>
							</td>
						</tr>
						<tr>
							...
							<td>
								<label for="mentoringSdate" style="margin-right: 10px;">
									<input id="mentoringSdate" name="mentoringSdate" type="text" style="border: 1px solid #e0e0e0; background-color: #fbfbfb;">	
								</label>
								<label style="margin-right: 10px;">~</label>
								<label for="mentoringFdate">
									<input id="mentoringFdate" name="mentoringFdate" type="text" style="border: 1px solid #e0e0e0; background-color: #fbfbfb;">	
								</label>
							</td>
						</tr>
						<tr>
							...
							<td>
								<input id="time" name="time" class="inputTxt01" style="width: 95%;" type="text" maxlength="100">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input id="peopleNumber" name="peopleNumber" class="inputTxt01" style="width: 95%;" type="text" maxlength="100">
							</td>
						</tr>
						<tr>
							<th scope="row"><label for="content"><span class="division">멘토링 세부 내용</span></label></th>
							<td>
								<textarea id="content" name="content" class="textarea" style="width: 95%; height: 120px;"></textarea>
							</td>
						</tr>
						<tr>
				...
				<!-- <input type="submit" value="등록하기" class="btn btn-theme btn-large btn-rounded e_wobble center" style="margin-top: 2em;"> -->
				</div>	
				</form>	
				<button class="btn btn-theme btn-large btn-rounded e_wobble center" style="margin-top: 2em;" onClick="insert();">등록하기</button>		
              </div>
            </article>
            <!-- author info -->
          </div>
  ...
  <script>
        $(function() {
            ...
            $('#recruitSdate').datepicker('setDate', 'today'); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)
            //To의 초기값을 내일로 설정
            $('#recruitFdate').datepicker('setDate', '+1D'); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)
          //From의 초기값을 오늘 날짜로 설정
            $('#mentoringSdate').datepicker('setDate', 'today'); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)
            //To의 초기값을 내일로 설정
            $('#mentoringFdate').datepicker('setDate', '+1D'); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)
        });
    </script>
