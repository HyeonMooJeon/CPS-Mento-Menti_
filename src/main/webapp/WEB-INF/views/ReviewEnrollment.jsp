...
<%
											...
											<tbody>
													...
													<tr>
														<th scpoe="row"><span>>제목</span></th>
												 		<td><input id="applyTitle" name="applyTitle"
															...
															style="width: 70%" type="text" maxlength="40"   
												...
												<tr>
													<td colspan="4"> 
													 ...
													 <input type="button" class="btn btn-outline-primary pull-right" onClick="fileupload();" value="전송" /> 
														<!--  <button class="btn btn-large btn-theme margintop10" type="submit">Submit message</button> -->
														<button onclick="location.href='/toring/Reviewlist'"
	...
	<script type="text/javascript">
		function load() {
			...
			/* 	var width = $('#file1').naturalWidth;
				var height = $('#file1').naturalHeight;
				alert(width); */
		...
		function fileupload() {
			...
			if (headname == "" || headname.length>40 || subject.length>500) {
				alert("제목과 내용을 다시 확인해 주세요(제목 입력,글자 수 확인)"); 
			} else {
				$.ajax({
					url : "/toring/insertreview",
					type : "post",
					data : formData,
					processData : false,
					contentType : false,
					...
					error : function(xhr, status, error) {
						alert("후기가 이미 등록 되어있습니다. 새로고침을 하시거나 다시 후기등록 버튼을 눌러주세요");
						location.href = "/toring/Reviewlist";
					}
				});
