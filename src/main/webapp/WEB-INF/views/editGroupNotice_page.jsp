...
<script>
//공지사항 조회
function groupStudyNotice(noticeNumber){
	$("#groupNumber").val('${notice.groupNumber}');
	$("#postNumber").val(noticeNumber);
	$("#mentor").val('${mentor}');
	$("#groupStudyNotice").submit();
}
//그룹 공지사항 추가
function edit(number){
	var formData= new FormData($("#editNotice")[0]);
	formData.append("groupNumber",number);
	formData.append("noticeNumber",'${notice.noticeNumber}');
	$.ajax({
		 url :"/toring/updateGroupNotice"
		 ,type:"post"
		 ,data:formData
         ,dataType:"json"
         ,processData : false
         ,contentType : false
	 	 ,success: function(data){
	 		 if(data.check==1){
	 			alert("게시글을 수정했습니다");
	 			groupStudyNotice('${notice.noticeNumber}');
	 		 }else{
	 			alert("데이터베이스 에러");
	 		 }	 		 
	 	  },error: function(){
...
<%
            ...
            <article>
              <div class="row">
          		...
          		<div class="span8">
          			...
          			<a href="javascript:void(0);" style="margin-top:20px;" class="btn btn-theme pull-right" onClick="groupStudyNotice(${notice.noticeNumber});">뒤로</a>
          		</div>
          	</article>
            <article>
              <div class="row" style="margin-left:0px;">
              	<form id="editNotice" name="editNotice" enctype="multipart/form-data">
				...
				<table class="bbsWriteA" summary="게시물 쓰기">
					...
					<tbody>
						<tr>
							<th scope="row"><label for="title"><span class="division">공지사항 제목</span></label></th>
							<td><input id="title" name="title" class="inputTxt01" style="width: 95%;" type="text" maxlength="100" value="${notice.title}"></td>
						</tr>
						<tr>
							<th scope="row"><label for="content"><span class="division">공지사항 내용</span></label></th>
							<td>
				...
				<button class="btn btn-theme btn-large btn-rounded center" style="margin-top: 2em;" onClick="edit(${notice.groupNumber});">수정하기</button>		
              </div>
            </article>
              </div>
            </article>
  ...
  <input type="hidden" id="memberNumber" name="memberNumber">
    ...
    <form id="groupStudyNotice" action="/toring/groupStudyNotice" method="post">
